plain_text =list(input("Entar to plain_text : ").upper())
...
def key_system(letters,key):# لي ترتيب احرفا لتي يدخلها المستخدم مع باقي الحرف بي الترتيب
    ...
    return key_letters
def cipher_text_usar (letters,plain_text,key):
    cipher_text =[]
    for plan in plain_text:
        x=0
        for le in letters:
            if plan ==le :
                cipher_text.append(key[x])
            x=x+1
    finel_cipher_text ="".join(cipher_text)
    return finel_cipher_text
t1 = key_usar(letters,key)
t2 = key_system(letters,t1)
t3 = cipher_text_usar (letters,plain_text,t2)
